...
class ShopProvider extends ChangeNotifier {
  ...
  static const _levelRewardDateKey = 'pp_practice_reward_date';
  static const _levelRewardCountKey = 'pp_practice_reward_count';
  static const _processedPurchasesKey = 'pp_processed_purchases';
  static const _lastSpinKey = 'pp_last_spin_date';
  static const _loginStreakKey = 'pp_login_streak';
  static const _lastStreakDateKey = 'pp_last_streak_date';
  static const _firstPurchaseBonusKey = 'pp_first_purchase_bonus_used';
  ...
  bool get hasSlowAudio => _ownedItems.contains('feat_slow_audio');
  bool get hasLargeType => _ownedItems.contains('feat_large_type');
  bool get hasExtraCategories => true;
  bool get hasQuizPlus => _ownedItems.contains('feat_quiz_plus');

  bool ownsLanguage(String langId) =>
      const {'ja', 'vi', 'es', 'fr'}.contains(langId) || _ownedItems.contains('lang_$langId');
  ...
  bool ownsItem(String id) {
    if (ShopCatalog.isDefaultId(id)) return true;
    final item = ShopCatalog.find(id);
  ...
  Future<bool> rewardForLevelComplete(int baseAmount) async {
    ...
    return _dailyReward(
      ...
      max: IapConstants.maxPracticeRewardsPerDay,
      amount: amount,
      messageKey: 'levelRewardEarned',
    );
  }

  Future<bool> rewardForPractice() {
    return rewardForLevelComplete(IapConstants.practiceReward);
  }

  Future<bool> rewardForFavorite() {
    return _dailyReward(
      dateKey: 'pp_fav_reward_date',
      countKey: 'pp_fav_reward_count',
      max: IapConstants.maxFavoriteRewardsPerDay,
      amount: effectiveCoinReward(IapConstants.favoriteReward),
      messageKey: 'favoriteRewardEarned',
    );
  }
