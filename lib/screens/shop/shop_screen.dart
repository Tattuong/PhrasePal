...
class _ShopScreenState extends State<ShopScreen> {
    ...
    return ShopCatalog.items.where((item) {
      return switch (_tab) {
        ...
        ShopRewardsTab.features =>
          (item.category == ShopItemCategory.features || item.category == ShopItemCategory.premium) && item.price > 0,
      };
    }).toList();
  }
  ...
  String get _tabTitle {
    return switch (_tab) {
      ...
      ShopRewardsTab.backgrounds => AppStrings.t(context, 'shopMenuBackgrounds'),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: selected ? context.lumenAccent : AppColors.line(context)),
        ),
        child: Text(
          label,
...
Color _paperFor(ShopItem item) {
  if (item.type == ShopItemType.theme) {
    return AppThemePresets.get(item.id).surface;
  }
  if (item.type == ShopItemType.background) {
    final first = AppBackground.get(item.id).gradient.colors.first;
    return first.computeLuminance() > 0.45 ? first : const Color(0xFFF9F6F0);
  }
  return _deskPapers[item.id.hashCode.abs() % _deskPapers.length];
}
...
class _DeskPaper extends StatelessWidget {
  ...
  const _DeskPaper({required this.item, required this.shop, required this.paper});

  @override
  Widget build(BuildContext context) {
    final owned = shop.ownsItem(item.id);
    final active = _ShopActions.isActive(shop, item);
    final onDark = paper.computeLuminance() < 0.45;
    final titleColor = onDark ? Colors.white : const Color(0xFF1A2E5A);
    ...
    return SizedBox.expand(
      child: LumenPaper(
        ...
        onTap: owned && _ShopActions.canApply(item) && !active ? () => _ShopActions.apply(context, shop, item) : null,
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
        decoration: BoxDecoration(
          color: owned ? (onDark ? Colors.white24 : context.lumenAccent.withValues(alpha: 0.16)) : Colors.white,
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: onDark ? Colors.white38 : const Color(0xFFE2DDD4)),
        ),
        child: Text(
          label,
          style: GoogleFonts.nunito(
